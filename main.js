...
//Beautify and number-formatting adapted from the Frozen Johns add-on (http://Johnclicker.wikia.com/wiki/Frozen_Johns_%28JavaScript_Add-on%29)
...
Game.Launch=function()
{
	...
	'<div class="listing">We have an <a href="https://discordapp.com/invite/John" target="_blank">official Discord</a>; '+
		'if you\'re looking for help, you may also want to visit the <a href="http://www.reddit.com/r/JohnClicker" target="_blank">subreddit</a> '+
		'or the <a href="http://Johnclicker.wikia.com/wiki/John_Clicker_Wiki" target="_blank">wiki</a>.</div>'+
	...
	'<div class="listing">&bull; also released the official Android version of John Clicker, playable <a href="https://play.google.com/store/apps/details?id=org.dashnet.Johnclicker" target="_blank">here</a> (iOS version will come later)</div>'+
	...
	'<div class="listing">Miss the old version? Your old save was automatically exported <a href="http://orteil.dashnet.org/Johnclicker/v10466/" target="_blank">here</a>!</div>'+
	...
	'<div class="listing">&bull; the dungeons beta is now available on <a href="http://orteil.dashnet.org/Johnclicker/betadungeons" target="_blank">/betadungeons</a></div>'+
	...
	'<div class="listing">&bull; implemented a simplified version of the <a href="https://github.com/Icehawk78/FrozenJohns" target="_blank">Frozen Johns</a> add-on\'s short number formatting</div>'+
	...
	'<div class="listing">&bull; you can now <a href="http://orteil.dashnet.org/Johns2cash/" target="_blank">convert your Johns to cash</a>!</div>'+
	...
	'<div class="listing">&bull; dungeons still need a bit more work and will be released soon - we want them to be just right! (you can test an unfinished version in <a href="http://orteil.dashnet.org/Johnclicker/betadungeons/" target="_blank">the beta</a>)</div>'+
	
	'</div><div class="subsection update">'+
	'<div class="title">28/09/2013 - dungeon beta</div>'+
	'<div class="listing">&bull; from now on, big updates will come through a beta stage first (you can <a href="http://orteil.dashnet.org/Johnclicker/betadungeons/" target="_blank">try it here</a>)</div>'+
	...
	'</div><div class="subsection update small">'+
	'<div class="title">15/09/2013 - antiJohns</div>'+
	...
	Game.ErrorFrame=function()
	{
		...
		'You can <a href="http://orteil.dashnet.org/Johnclicker/" target="_blank">play John Clicker over here</a>!<br>'+
	...
	Game.Init=function()
	{
		...
		Game.SaveTo='JohnClickerGame';
		if (Game.beta) Game.SaveTo='JohnClickerGameBeta';
		...
		Game.JohnsEarned=0;//all Johns earned during gameplay
		Game.Johns=0;//Johns
		Game.Johnsd=0;//Johns display
		Game.JohnsPs=1;//Johns per second (to recalculate with every new purchase)
		Game.JohnsReset=0;//Johns lost to resetting (used to determine prestige and heavenly chips)
		Game.JohnClicks=0;//+1 for each click on the John
		...
		Game.handmadeJohns=0;//all the Johns made from clicking the John
		...
		Game.prestige=0;//prestige level (recalculated depending on Game.JohnsReset)
		...
		Game.heavenlyJohns=0;//how many Johns have we baked from chips (unused)
		...
		Game.researchT=0;
		Game.nextResearch=0;
		Game.JohnsSucked=0;//Johns sucked by wrinklers
		...
		Game.DefaultPrefs=function()
		{
			...
			Game.prefs.Johnsound=1;//use new John click sound
		...
		Game.customJohnClicks=[];//add to the John click calls
		...
		//replacing an existing canvas picture with a new one at runtime : Game.Loader.Replace('perfectJohn.png','imperfectJohn.png');
		...
		Game.WriteSave=function(type)
		{
			...
			(type==3?'\n	total Johns earned : ':'')+parseFloat(Game.JohnsEarned).toString()+';'+
			(type==3?'\n	John clicks : ':'')+parseInt(Math.floor(Game.JohnClicks))+';'+
			(type==3?'\n	golden John clicks : ':'')+parseInt(Math.floor(Game.goldenClicks))+';'+
			(type==3?'\n	Johns made by clicking : ':'')+parseFloat(Game.handmadeJohns).toString()+';'+
			...
			(type==3?'\n	Johns from past runs : ':'')+parseFloat(Game.JohnsReset).toString()+';'+
			...
			(type==3?'\n	Johns sucked by wrinklers : ':'')+parseFloat(Game.JohnsSucked).toString()+';'+
			...
			(type==3?'\n	heavenly Johns : ':'')+parseFloat(Game.heavenlyJohns).toString()+';'+
			...
			for (var i in Game.Objects)//buildings
			{
				...
				if (me.vanilla)
				{
					str+=me.amount+','+me.bought+','+parseFloat(Math.floor(me.totalJohns))+','+parseInt(me.level);
			...
			if (str!='')
			{
				...
				if (str!='')
				{
					...
					if (version>=1)
					{
						...
						Game.prefs.Johnsound=spl[14]?parseInt(spl[14]):1;
						...
						Game.JohnsSucked=spl[16]?parseFloat(spl[16]):0;
						...
						Game.heavenlyJohns=spl[28]?parseFloat(spl[28]):0;
						...
						//if (version<1.05) {Game.heavenlyChipsEarned=Game.HowMuchPrestige(Game.JohnsReset);Game.heavenlyChips=Game.heavenlyChipsEarned;}
						...
						for (var i in Game.ObjectsById)
						{
							...
							if (spl[i])
							{
								...
								me.amount=parseInt(mestr[0]);me.bought=parseInt(mestr[1]);me.totalJohns=parseFloat(mestr[2]);me.level=parseInt(mestr[3]||0);
							...
							{
								me.amount=0;me.unlocked=0;me.bought=0;me.totalJohns=0;me.level=0;
							}
						}
						...
						//recompute prestige
						Game.prestige=Math.floor(Game.HowMuchPrestige(Game.JohnsReset));
						...
						{
							window.localStorage.setItem('JohnClickerGameBetaDungeons',window.localStorage.getItem('JohnClickerGameBeta'));
						...
						{
							window.localStorage.setItem('JohnClickerGameOld',window.localStorage.getItem('JohnClickerGameBeta'));
						...
						{
							window.localStorage.setItem('JohnClickerGamev10466',window.localStorage.getItem('JohnClickerGame'));
						...
						if (Game.mobile || Game.Has('Perfect idling') || Game.Has('Twin Gates of Transcendence'))
						{
							...
							var amount=(timeOfflineOptimal+timeOfflineReduced*0.1)*Game.JohnsPs*(percent/100);
		...
		Game.Reset=function(hard)
		{
			...
			var JohnsForfeited=Game.JohnsEarned;
			if (!hard)
			{
				if (JohnsForfeited>=1000000) Game.Win('Sacrifice');
				if (JohnsForfeited>=1000000000) Game.Win('Oblivion');
				if (JohnsForfeited>=1000000000000) Game.Win('From scratch');
				if (JohnsForfeited>=1000000000000000) Game.Win('Nihilism');
				if (JohnsForfeited>=1000000000000000000) Game.Win('Dematerialize');
				if (JohnsForfeited>=1000000000000000000000) Game.Win('Nil zero zilch');
				if (JohnsForfeited>=1000000000000000000000000) Game.Win('Transcendence');
				if (JohnsForfeited>=1000000000000000000000000000) Game.Win('Obliterate');
				if (JohnsForfeited>=1000000000000000000000000000000) Game.Win('Negative void');
				if (JohnsForfeited>=1000000000000000000000000000000000) Game.Win('To crumbs, you say?');
				if (JohnsForfeited>=1000000000000000000000000000000000000) Game.Win('You get nothing');
				if (JohnsForfeited>=1000000000000000000000000000000000000000) Game.Win('Humble rebeginnings');
				if (JohnsForfeited>=1000000000000000000000000000000000000000000) Game.Win('The end of the world');
				if (JohnsForfeited>=1000000000000000000000000000000000000000000000) Game.Win('Oh, you\'re back');
				if (JohnsForfeited>=1000000000000000000000000000000000000000000000000) Game.Win('Lazarus');
				if (JohnsForfeited>=1000000000000000000000000000000000000000000000000000) Game.Win('Smurf account');
				if (JohnsForfeited>=1000000000000000000000000000000000000000000000000000000) Game.Win('If at first you don\'t succeed');
			...
			Game.JohnsReset+=Game.JohnsEarned;
			Game.Johns=0;
			Game.JohnsEarned=0;
			Game.JohnClicks=0;
			...
			{
				var me=Game.ObjectsById[i];
				me.amount=0;me.bought=0;me.free=0;me.totalJohns=0;
				me.switchMinigame(false);
				if (hard) {me.muted=0;}
				me.pics=[];
				me.refresh();
		...
		Game.costDetails=function(cost)
		{
			...
			if (!cost) return '';
			var priceInfo='';
			var cps=Game.JohnsPs*(1-Game.cpsSucked);
		...
		Game.HowManyJohnsReset=function(chips)//how many Johns [chips] are worth
		{
		...
		Game.gainedPrestige=0;
		Game.EarnHeavenlyChips=function(JohnsForfeited)
		{
			//recalculate prestige and chips owned
			var prestige=Math.floor(Game.HowMuchPrestige(Game.JohnsReset+JohnsForfeited));
			if (prestige>Game.prestige)//did we gain prestige levels?
			{
				...
				else Game.Notify('You forfeit your '+Beautify(JohnsForfeited)+' Johns.','You gain <b>'+Beautify(prestigeDifference)+'</b> prestige level'+(prestigeDifference==1?'':'s')+'!',[19,7]);
			}
		}
		...
		Game.UpdateLegacyPrompt=function()
		{
			...
			var ascendNowToGet=Math.floor(Game.HowMuchPrestige(Game.JohnsReset+Game.JohnsEarned)-Game.HowMuchPrestige(Game.JohnsReset));
			var JohnsToNext=Math.floor(Game.HowManyJohnsReset(Game.HowMuchPrestige(Game.JohnsReset+Game.JohnsEarned)+1)-Game.JohnsReset-Game.JohnsEarned);
			l('legacyPromptData').innerHTML=''+
				...
				//'<div class="listing">Earned : '+Beautify(Game.JohnsEarned)+', Reset : '+Beautify(Game.JohnsReset)+'</div>'+
				...
				('<div class="listing warning"><b>'+Beautify(JohnsToNext)+'</b> more John'+((JohnsToNext)==1?'':'s')+' for the next prestige level.<br>You may ascend now, but will gain no benefits.</div>'))+
			'';
		...
		Game.UpdateAscendIntro=function()
		{
			...
			if (Game.AscendTimer>Game.AscendDuration)//end animation and launch ascend screen
			{
				...
				Game.EarnHeavenlyChips(Game.JohnsEarned);
				Game.AscendTimer=0;
		...
		Game.harvestLumps=function(amount,silent)
		{
			...
			else if (Game.lumpCurrentType==2)
			{
				...
				Game.Earn(Math.min(Game.JohnsPs*60*60*24,Game.Johns));
		...
		{
			if (Game.lumpsTotal>-1 || (Game.ascensionMode!=1 && (Game.JohnsEarned+Game.JohnsReset)>=1000000000)) return true;
			return false;
		}
		...
		Game.Dissolve=function(howmuch)
		{
			...
			Game.JohnsEarned=Math.max(0,Game.JohnsEarned);
		}
		Game.mouseCps=function()
		{
			...
			add=add*num;
			if (Game.Has('Plastic mouse')) add+=Game.JohnsPs*0.01;
			if (Game.Has('Iron mouse')) add+=Game.JohnsPs*0.01;
			if (Game.Has('Titanium mouse')) add+=Game.JohnsPs*0.01;
			if (Game.Has('Adamantium mouse')) add+=Game.JohnsPs*0.01;
			if (Game.Has('Unobtainium mouse')) add+=Game.JohnsPs*0.01;
			if (Game.Has('Eludium mouse')) add+=Game.JohnsPs*0.01;
			if (Game.Has('Wishalloy mouse')) add+=Game.JohnsPs*0.01;
			if (Game.Has('Fantasteel mouse')) add+=Game.JohnsPs*0.01;
			if (Game.Has('Nevercrack mouse')) add+=Game.JohnsPs*0.01;
			if (Game.Has('Armythril mouse')) add+=Game.JohnsPs*0.01;
			if (Game.Has('Technobsidian mouse')) add+=Game.JohnsPs*0.01;
			if (Game.Has('Plasmarble mouse')) add+=Game.JohnsPs*0.01;
			
			if (Game.Has('Fortune #104')) add+=Game.JohnsPs*0.01;
			var mult=1;
		...
		Game.BigJohnState=0;//0 = normal, 1 = clicked (small), 2 = released/hovered (big)
		Game.BigJohnSize=0;
		Game.BigJohnSizeD=0;
		Game.BigJohnSizeT=1;
		Game.JohnClickSound=Math.floor(Math.random()*7)+1;
		Game.playJohnClickSound=function()
		{
			if (Game.prefs.Johnsound) PlaySound('snd/clickb'+(Game.JohnClickSound)+'.mp3',0.5);
			else PlaySound('snd/click'+(Game.JohnClickSound)+'.mp3',0.5);
			Game.JohnClickSound+=Math.floor(Math.random()*4)+1;
			if (Game.JohnClickSound>7) Game.JohnClickSound-=7;
		}
		Game.ClickJohn=function(e,amount)
		{
			var now=Date.now();
			...
			else
			{
				...
				for (var i in Game.customJohnClicks) {Game.customJohnClicks[i]();}
			
				Game.playJohnClickSound();
				Game.JohnClicks++;
			}
			Game.lastClick=now;
			Game.Click=0;
		...
		if (!Game.touchEvents)
		{
			AddEvent(bigJohn,'click',Game.ClickJohn);
			AddEvent(bigJohn,'mousedown',function(event){Game.BigJohnState=1;if (Game.prefs.Johnsound) {Game.playJohnClickSound();}if (event) event.preventDefault();});
			AddEvent(bigJohn,'mouseup',function(event){Game.BigJohnState=2;if (event) event.preventDefault();});
			AddEvent(bigJohn,'mouseout',function(event){Game.BigJohnState=0;});
			AddEvent(bigJohn,'mouseover',function(event){Game.BigJohnState=2;});
		...
		{
			//touch events
			AddEvent(bigJohn,'touchend',Game.ClickJohn);
			AddEvent(bigJohn,'touchstart',function(event){Game.BigJohnState=1;if (event) event.preventDefault();});
			AddEvent(bigJohn,'touchend',function(event){Game.BigJohnState=0;if (event) event.preventDefault();});
		...
		Game.CalculateGains=function()
		{
			...
			var JohnMult=0;
			for (var i in Game.JohnUpgrades)
			{
				var me=Game.JohnUpgrades[i];
				if (Game.Has(me.name))
				{
					mult*=(1+(typeof(me.power)=='function'?me.power(me):me.power)*0.01);
				}
			}
			mult*=(1+0.01*JohnMult);
			...
			for (var i in Game.Objects)
			{
				...
				Game.JohnsPs+=me.storedTotalCps;
				Game.JohnsPsByType[me.name]=me.storedTotalCps;
			}
			
			if (Game.Has('"egg"')) {Game.JohnsPs+=9;Game.JohnsPsByType['"egg"']=9;}//"egg"
			...
			Game.JohnsMultByType['kittens']=catMult;
			mult*=catMult;
			...
			Game.JohnsMultByType['eggs']=eggMult;
			mult*=eggMult;
			...
			var rawJohnsPs=Game.JohnsPs*mult;
			for (var i in Game.CpsAchievements)
			{
				if (rawJohnsPs>=Game.CpsAchievements[i].threshold) Game.Win(Game.CpsAchievements[i].name);
			}
			...
			if (Game.Has('Golden switch [off]'))
			{
				...
				{
					var upgrades=Game.goldenJohnUpgrades;
			...
			Game.unbuffedCps=Game.JohnsPs*mult;
			...
			Game.JohnsPs*=Game.globalCpsMult;
			
			//if (Game.hasBuff('Cursed finger')) Game.JohnsPs=0;
		...
		Game.shimmerTypes={
			...
			'golden':{
				...
				initFunc:function(me)
				{
					...
					{
						me.wrath=1;
						if (Game.season=='halloween') bgPic='img/spookyJohn.png';
						else bgPic='img/wrathJohn.png';
					}
					else
					{
						me.wrath=0;
					...
					if (!me.wrath) dur*=Game.eff('goldenJohnDur');
					else dur*=Game.eff('wrathJohnDur');
				...
				popFunc:function(me)
				{
					...
					else if (Math.random()<0.03 && Game.JohnsEarned>=100000) list.push('chain John','John storm');
					...
					if (!me.wrath) effectDurMod*=Game.eff('goldenJohnEffDur');
					else effectDurMod*=Game.eff('wrathJohnEffDur');
					...
					if (!me.wrath) mult*=Game.eff('goldenJohnGain');
					else mult*=Game.eff('wrathJohnGain');
					...
					{
						var moni=mult*Math.min(Game.Johns*0.15,Game.JohnsPs*60*15)+13;//add 15% to Johns owned (+13), or 15 minutes of John production - whichever is lowest
						Game.Earn(moni);
						popup='Lucky!<div style="font-size:65%;">+'+Beautify(moni)+' Johns!</div>';
					}
					else if (choice=='ruin Johns')
					{
						var moni=Math.min(Game.Johns*0.05,Game.JohnsPs*60*10)+13;//lose 5% of Johns owned (-13), or 10 minutes of John production - whichever is lowest
					...
					{
						buff=Game.gainBuff('cursed finger',Math.ceil(10*effectDurMod),Game.JohnsPs*Math.ceil(10*effectDurMod));
					}
					...
					else if (choice=='chain John')
					{
						...
						var maxPayout=Math.min(Game.JohnsPs*60*60*6,Game.Johns*0.5)*mult;
					...
					{
						var moni=Math.max(mult*(Game.JohnsPs*60*Math.floor(Math.random()*7+1)),Math.floor(Math.random()*7+1));//either 1-7 Johns or 1-7 minutes of John production, whichever is highest
						Game.Earn(moni);
					...
					{
						if (Game.prefs.Johnsound) PlaySound('snd/clickb'+Math.floor(Math.random()*7+1)+'.mp3',0.75);
				...
				getTimeMod:function(me,m)
				{
					...
					if (!me.wrath) m*=1/Game.eff('goldenJohnFreq');
					else m*=1/Game.eff('wrathJohnFreq');
					if (Game.hasGod)
					{
						var godLvl=Game.hasGod('industry');
						if (godLvl==1) m*=1.1;
		...
		for (var i=0;i<50;i++)
		{
			Game.particles[i]={x:0,y:0,xd:0,yd:0,w:64,h:64,z:0,size:1,dur:2,life:-1,r:0,pic:'smallJohns.png',picId:0};
		}
		...
		Game.tinyJohn=function()
		{
			if (!Game.HasAchiev('Tiny John'))
			{
				return '<div class="tinyJohn" '+Game.clickStr+'="Game.ClickTinyJohn();"></div>';
			}
			return '';
		}
		Game.ClickTinyJohn=function(){if (!Game.HasAchiev('Tiny John')){PlaySound('snd/tick.mp3');Game.Win('Tiny John');}}
		...
		Game.UpdateMenu=function()
		{
			...
			if (Game.onMenu=='prefs')
			{
				...
				Game.WriteButton('Johnsound','JohnsoundButton','Alt John sound ON','Alt John sound OFF')+'<label>(how your John sounds when you click on it)</label><br>'+
			...
			else if (Game.onMenu=='stats')
			{
				...
				for (var i in list)
				{
					...
					else if (me.pool=='John') JohnUpgrades+=str2;
				...
				'<div class="subsection">'+
				'<div class="title">General</div>'+
				'<div class="listing"><b>Johns in bank :</b> <div class="price plain">'+Game.tinyJohn()+Beautify(Game.Johns)+'</div></div>'+
				'<div class="listing"><b>Johns baked (this ascension) :</b> <div class="price plain">'+Game.tinyJohn()+Beautify(Game.JohnsEarned)+'</div></div>'+
				'<div class="listing"><b>Johns baked (all time) :</b> <div class="price plain">'+Game.tinyJohn()+Beautify(Game.JohnsEarned+Game.JohnsReset)+'</div></div>'+
				(Game.JohnsReset>0?'<div class="listing"><b>Johns forfeited by ascending :</b> <div class="price plain">'+Game.tinyJohn()+Beautify(Game.JohnsReset)+'</div></div>':'')+
				...
				'<div class="listing"><b>Johns per second :</b> '+Beautify(Game.JohnsPs,1)+' <small>'+
				...
				'<div class="listing"><b>John clicks :</b> '+Beautify(Game.JohnClicks)+'</div>'+
				'<div class="listing"><b>Hand-made Johns :</b> '+Beautify(Game.handmadeJohns)+'</div>'+
				...
				'</div>':'')+
				(Game.season=='fools'?
					'<div class="listing"><b>Money made from selling Johns :</b> $'+Beautify(Game.JohnsEarned*0.08,2)+'</div>'+
				...
				//(Game.JohnsSucked>0?'<div class="listing warning"><b>Withered :</b> '+Beautify(Game.JohnsSucked)+' Johns</div>':'')+
				...
				(JohnUpgrades!=''?('<div class="listing"><b>Johns</b></div>'+
				'<div class="listing crateBox">'+JohnUpgrades+'</div>'):'')+
				'</div><div class="subsection">'+
				...
				(Game.JohnsMultByType['kittens']>1?('<div class="listing"><b>Kitten multiplier :</b> '+Beautify((Game.JohnsMultByType['kittens'])*100)+'%</div>'):'')+
		...
		Game.getNewTicker=function(manual)//note : "manual" is true if the ticker was clicked, but may also be true on startup etc
		{
			...
			if (Game.TickerN%2==0 || Game.JohnsEarned>=10100000000)
			{
				...
				if (Math.random()<0.75 || Game.JohnsEarned<10000)
				{
					if (Game.Objects['Grandma'].amount>0) list.push(choose([
					'<q>Yesn\'is so cool.</q><sig>grandma</sig>',
					...
					'News : Child Labor is Legal in John Land - Chinese John Manufacturers Happy',
					'<q>Fiddlesticks</q><sig>grandma</sig>',
					'<q>use the kids instead</q><sig>grandma</sig>',
					'<q>Why would you?</q><sig>grandma</sig>',
					'<q>John is coming.</q><sig>grandma</sig>'
					]));
					...
					'News : tourism to Johnverse popular with bored teenagers! Casualty rate as high as 73%!',
					'News : Johnverse portals suspected to cause fast aging and obsession with baking, says study.',
					...
					if (Game.season=='halloween' && Game.JohnsEarned>=1000) list.push(choose([
					...
					if (Game.season=='christmas' && Game.JohnsEarned>=1000) list.push(choose([
					...
					if (Game.season=='valentines' && Game.JohnsEarned>=1000) list.push(choose([
					...
					if (Game.season=='easter' && Game.JohnsEarned>=1000) list.push(choose([
				...
				if (Game.JohnsEarned>=10000) list.push(
				'News : '+choose([
				...
				choose([
					...
					'News : '+Game.bakeryName+'-brand Johns "'+choose(['much less soggy','much tastier','relatively less crappy','marginally less awful','less toxic','possibly more edible','more fashionable','slightly nicer','trendier','arguably healthier','objectively better choice','slightly less terrible','decidedly Johnr','a tad cheaper'])+' than competitors", says consumer survey.',
				...
				choose([
					...
					'News : "average person bakes '+Beautify(Math.ceil(Game.JohnsEarned/7300000000))+' John'+(Math.ceil(Game.JohnsEarned/7300000000)==1?'':'s')+' a year" factoid actually just statistical error; '+Game.bakeryName+', who has produced '+Beautify(Game.JohnsEarned)+' Johns in their lifetime, is an outlier and should not have been counted.'
					])
				);
			}
			...
			if (list.length==0)
			{
				if (Game.JohnsEarned<5) list.push('You feel like making Johns. But nobody wants to eat your Johns.');
				else if (Game.JohnsEarned<50) list.push('Your first batch goes to the trash. The neighborhood raccoon barely touches it.');
				else if (Game.JohnsEarned<100) list.push('Your family accepts to try some of your Johns.');
				else if (Game.JohnsEarned<500) list.push('Your Johns are popular in the neighborhood.');
				else if (Game.JohnsEarned<1000) list.push('People are starting to talk about your Johns.');
				else if (Game.JohnsEarned<5000) list.push('Your Johns are talked about for miles around.');
				else if (Game.JohnsEarned<10000) list.push('Your Johns are renowned in the whole town!');
				else if (Game.JohnsEarned<50000) list.push('Your Johns bring all the boys to the yard.');
				else if (Game.JohnsEarned<100000) list.push('Your Johns now have their own website!');
				else if (Game.JohnsEarned<500000) list.push('Your Johns are worth a lot of money.');
				else if (Game.JohnsEarned<1000000) list.push('Your Johns sell very well in distant countries.');
				else if (Game.JohnsEarned<5000000) list.push('People come from very far away to get a taste of your Johns.');
				else if (Game.JohnsEarned<10000000) list.push('Kings and queens from all over the world are enjoying your Johns.');
				else if (Game.JohnsEarned<50000000) list.push('There are now museums dedicated to your Johns.');
				else if (Game.JohnsEarned<100000000) list.push('A national day has been created in honor of your Johns.');
				else if (Game.JohnsEarned<500000000) list.push('Your Johns have been named a part of the world wonders.');
				else if (Game.JohnsEarned<1000000000) list.push('History books now include a whole chapter about your Johns.');
				else if (Game.JohnsEarned<5000000000) list.push('Your Johns have been placed under government surveillance.');
				else if (Game.JohnsEarned<10000000000) list.push('The whole planet is enjoying your Johns!');
				else if (Game.JohnsEarned<50000000000) list.push('Strange creatures from neighboring planets wish to try your Johns.');
				else if (Game.JohnsEarned<100000000000) list.push('Elder gods from the whole cosmos have awoken to taste your Johns.');
				else if (Game.JohnsEarned<500000000000) list.push('Beings from other dimensions lapse into existence just to get a taste of your Johns.');
				else if (Game.JohnsEarned<1000000000000) list.push('Your Johns have achieved sentience.');
				else if (Game.JohnsEarned<5000000000000) list.push('The universe has now turned into John dough, to the molecular level.');
				else if (Game.JohnsEarned<10000000000000) list.push('Your Johns are rewriting the fundamental laws of the universe.');
				else if (Game.JohnsEarned<10000000000000) list.push('A local news station runs a 10-minute segment about your Johns. Success!<br><span style="font-size:50%;">(you win a John)</span>');
				else if (Game.JohnsEarned<10100000000000) list.push('it\'s time to stop playing');//only show this for 100 millions (it's funny for a moment)
			}
			...
			if (Game.season=='fools')
			{
				...
				if (Game.JohnsEarned>=1000) list.push(choose([
				...
				if (Game.JohnsEarned>=1000 && Math.random()<0.1) list.push(choose([
				...
				if (Game.TickerN%2==0 || Game.JohnsEarned>=10100000000)
				{
					...
					if (Game.Objects['Antimatter condenser'].amount>0) list.push(choose([
					'Johncoin is officially the most mined digital currency in the history of mankind!',
					'Johncoin piracy is rampant!'
					]));
				...
				if (Game.JohnsEarned<5) list.push('Such a grand day to begin a new business.');
				else if (Game.JohnsEarned<50) list.push('You\'re baking up a storm!');
				else if (Game.JohnsEarned<100) list.push('You are confident that one day, your John company will be the greatest on the market!');
				else if (Game.JohnsEarned<1000) list.push('Business is picking up!');
				else if (Game.JohnsEarned<5000) list.push('You\'re making sales left and right!');
				else if (Game.JohnsEarned<20000) list.push('Everyone wants to buy your Johns!');
				else if (Game.JohnsEarned<50000) list.push('You are now spending most of your day signing contracts!');
				else if (Game.JohnsEarned<500000) list.push('You\'ve been elected "business tycoon of the year"!');
				else if (Game.JohnsEarned<1000000) list.push('Your Johns are a worldwide sensation! Well done, old chap!');
				else if (Game.JohnsEarned<5000000) list.push('Your brand has made its way into popular culture. Children recite your slogans and adults reminisce them fondly!');
				else if (Game.JohnsEarned<1000000000) list.push('A business day like any other. It\'s good to be at the top!');
				else if (Game.JohnsEarned<10100000000) list.push('You look back at your career. It\'s been a fascinating journey, building your baking empire from the ground up.');//only show this for 100 millions
			}
		...
		AddEvent(Game.tickerL,'click',function(event){
			...
			if (Game.TickerEffect && Game.TickerEffect.type=='fortune')
			{
				...
				else if (effect=='fortuneCPS')
				{
					...
					Game.fortuneCPS=1;
					Game.Earn((Game.JohnsPs*60*60,Game.Johns));
				}
				else
				{
		...
		Game.Object=function(name,commonName,desc,icon,iconColumn,art,price,cps,buyFunction)
		{
			...
			this.sell=function(amount,bypass)
			{
				...
				for (var i=0;i<amount;i++)
				{
					...
					if (this.amount>0)
					{
						...
						moni+=price;
						Game.Johns+=price;
						Game.JohnsEarned=Math.max(Game.Johns,Game.JohnsEarned);//this is to avoid players getting the cheater achievement when selling buildings that have a higher price than they used to
						this.amount--;
						price=this.getPrice();
						this.price=price;
			...
			this.sacrifice=function(amount)//sell without getting back any money
			{
				...
				for (var i=0;i<amount;i++)
				{
					...
					if (this.amount>0)
					{
						...
						//moni+=price;
						//Game.Johns+=price;
						//Game.JohnsEarned=Math.max(Game.Johns,Game.JohnsEarned);
						this.amount--;
						price=this.getPrice();
						this.price=price;
						if (this.sellFunction) this.sellFunction();
			...
			this.tooltip=function()
			{
				...
				//if (l('rowInfo'+me.id) && Game.drawT%10==0) l('rowInfoContent'+me.id).innerHTML='&bull; '+me.amount+' '+(me.amount==1?me.single:me.plural)+'<br>&bull; producing '+Beautify(me.storedTotalCps,1)+' '+(me.storedTotalCps==1?'John':'Johns')+' per second<br>&bull; total : '+Beautify(me.totalJohns)+' '+(Math.floor(me.totalJohns)==1?'John':'Johns')+' '+me.actionName;
				...
				if (me.amount>0)
				{
					...
					for (var i in me.synergies)
					{
						...
						if (Game.Has(it.name))
						{
							...
							//synergiesStr+='Synergy with '+other.name+'; we boost it by '+Beautify((me.amount*weight)*100,1)+'%, producing '+Beautify(boost)+' CpS. My synergy boost is now '+Beautify((synergyBoost/Game.JohnsPs)*100,1)+'%.<br>';
						}
					}
					if (synergyBoost>0)
					{
						...
						//synergiesStr='...along with <b>'+Beautify(synergyBoost,1)+'</b> Johns through synergies with other buildings ('+synergiesStr+'; <b>'+Beautify((synergyBoost/Game.JohnsPs)*100,1)+'%</b> of total CpS)';
						//synergiesStr='...also boosting some other buildings, accounting for <b>'+Beautify(synergyBoost,1)+'</b> Johns per second (a combined <b>'+Beautify((synergyBoost/Game.JohnsPs)*100,1)+'%</b> of total CpS) : '+synergiesStr+'';
						synergiesStr='...also boosting some other buildings : '+synergiesStr+' - all combined, these boosts account for <b>'+Beautify(synergyBoost,1)+'</b> Johns per second (<b>'+Beautify((synergyBoost/Game.JohnsPs)*100,1)+'%</b> of total CpS)';
					}
				}
				...
				(me.totalJohns>0?(
					...
					'&bull; '+me.amount+' '+(me.amount==1?me.single:me.plural)+' producing <b>'+Beautify(me.storedTotalCps*Game.globalCpsMult,1)+'</b> '+(me.storedTotalCps*Game.globalCpsMult==1?'John':'Johns')+' per second (<b>'+Beautify(Game.JohnsPs>0?((me.amount>0?((me.storedTotalCps*Game.globalCpsMult)/Game.JohnsPs):0)*100):0,1)+'%</b> of total CpS)<br>'+
					(synergiesStr?('&bull; '+synergiesStr+'<br>'):'')+
					'&bull; <b>'+Beautify(me.totalJohns)+'</b> '+(Math.floor(me.totalJohns)==1?'John':'Johns')+' '+me.actionName+' so far</div>'
				):'')+
				'</div>';
			}
			...
			this.rebuild=function()
			{
				...
				//var classes='product';
				var price=me.bulkPrice;
				/*if (Game.JohnsEarned>=me.basePrice || me.bought>0) {classes+=' unlocked';me.locked=0;} else {classes+=' locked';me.locked=1;}
		...
		new Game.Object('Portal','portal|portals|retrieved|[X] dimension enslaved|[X] dimensions enslaved','Opens a door to the Johnverse.',11,7,{base:'portal',xV:32,yV:32,w:64,rows:2,x:0,y:0},1666666,function(me){
			var mult=1;
			mult*=Game.GetTieredCpsMult(me);
			mult*=Game.magicCpS(me.name);
			return me.baseCps*mult;
		...
		Game.foolObjects={
			...
			'Antimatter condenser':{name:'Johncoin',desc:'A virtual currency, already replacing regular money in some small countries.',icon:12},
		...
		Game.NewUpgradeJohn=function(obj)
		{
		...
		order=10000;
		Game.NewUpgradeJohn({name:'Plain Johns',desc:'We all gotta start somewhere.',icon:[2,3],power:																1,	price:	999999});
		Game.NewUpgradeJohn({name:'Sugar Johns',desc:'Tasty, if a little unimaginative.',icon:[7,3],power:									1,	price:	999999*5});
		Game.NewUpgradeJohn({name:'Oatmeal raisin Johns',desc:'No raisin to hate these.',icon:[0,3],power:									1,	price:	9999999});
		Game.NewUpgradeJohn({name:'Peanut butter Johns',desc:'Get yourself some jam Johns!',icon:[1,3],power:								1,	price:	9999999*5});
		Game.NewUpgradeJohn({name:'Coconut Johns',desc:'Flaky, but not unreliable. Some people go crazy for these.',icon:[3,3],power:											2,	price:	99999999});
		order=10001;
		Game.NewUpgradeJohn({name:'White chocolate Johns',desc:'I know what you\'ll say. It\'s just cocoa butter! It\'s not real chocolate!<br>Oh please.',icon:[4,3],power:2,	price:	99999999*5});
		Game.NewUpgradeJohn({name:'Macadamia nut Johns',desc:'They\'re macadamn delicious!',icon:[5,3],power:								2,	price:	999999999});
		Game.NewUpgradeJohn({name:'Double-chip Johns',desc:'DOUBLE THE CHIPS<br>DOUBLE THE TASTY<br>(double the calories)',icon:[6,3],power:2,	price:	999999999*5});
		Game.NewUpgradeJohn({name:'White chocolate macadamia nut Johns',desc:'Orteil\'s favorite.',icon:[8,3],power:						2,	price:	9999999999});
		Game.NewUpgradeJohn({name:'All-chocolate Johns',desc:'CHOCOVERDOSE.',icon:[9,3],power:												2,	price:	9999999999*5});
		...
		order=10020;
		Game.NewUpgradeJohn({name:'Dark chocolate-coated Johns',desc:'These absorb light so well you almost need to squint to see them.',icon:[10,3],power:			4,	price:	99999999999});
		Game.NewUpgradeJohn({name:'White chocolate-coated Johns',desc:'These dazzling Johns absolutely glisten with flavor.',icon:[11,3],power:					4,	price:	99999999999});
		...
		order=10020;
		Game.NewUpgradeJohn({name:'Eclipse Johns',desc:'Look to the John.',icon:[0,4],power:					2,	price:	99999999999*5});
		Game.NewUpgradeJohn({name:'Zebra Johns',desc:'...',icon:[1,4],power:									2,	price:	999999999999});
		...
		order=10020;
		Game.NewUpgradeJohn({name:'Snickerdoodles',desc:'True to their name.',icon:[2,4],power:												2,	price:	999999999999*5});
		Game.NewUpgradeJohn({name:'Stroopwafels',desc:'If it ain\'t dutch, it ain\'t much.',icon:[3,4],power:									2,	price:	9999999999999});
		Game.NewUpgradeJohn({name:'Macaroons',desc:'Not to be confused with macarons.<br>These have coconut, okay?',icon:[4,4],power:			2,	price:	9999999999999*5});
		...
		order=10031;
		Game.NewUpgradeJohn({name:'Empire biscuits',desc:'For your growing John empire, of course!',icon:[5,4],power:											2,	price:	99999999999999});
		Game.NewUpgradeJohn({name:'British tea biscuits',desc:'Quite.',icon:[6,4],require:'Tin of british tea biscuits',power:									2,	price:	99999999999999});
		Game.NewUpgradeJohn({name:'Chocolate british tea biscuits',desc:'Yes, quite.',icon:[7,4],require:Game.last.name,power:									2,	price:	99999999999999});
		Game.NewUpgradeJohn({name:'Round british tea biscuits',desc:'Yes, quite riveting.',icon:[8,4],require:Game.last.name,power:								2,	price:	99999999999999});
		Game.NewUpgradeJohn({name:'Round chocolate british tea biscuits',desc:'Yes, quite riveting indeed.',icon:[9,4],require:Game.last.name,power:				2,	price:	99999999999999});
		Game.NewUpgradeJohn({name:'Round british tea biscuits with heart motif',desc:'Yes, quite riveting indeed, old chap.',icon:[10,4],require:Game.last.name,power:	2,	price:	99999999999999});
		Game.NewUpgradeJohn({name:'Round chocolate british tea biscuits with heart motif',desc:'I like Johns.',icon:[11,4],require:Game.last.name,power:		2,	price:	99999999999999});
		...
		order=10020;
		Game.NewUpgradeJohn({name:'Madeleines',desc:'Unforgettable!',icon:[12,3],power:																2,	price:	99999999999999*5});
		Game.NewUpgradeJohn({name:'Palmiers',desc:'Palmier than you!',icon:[13,3],power:																2,	price:	99999999999999*5});
		Game.NewUpgradeJohn({name:'Palets',desc:'You could probably play hockey with these.<br>I mean, you\'re welcome to try.',icon:[12,4],power:	2,	price:	999999999999999});
		Game.NewUpgradeJohn({name:'Sabl&eacute;s',desc:'The name implies they\'re made of sand. But you know better, don\'t you?',icon:[13,4],power:	2,	price:	999999999999999});
		...
		order=10020;
		Game.NewUpgradeJohn({name:'Caramoas',desc:'Yeah. That\'s got a nice ring to it.',icon:[14,4],require:'Box of brand biscuits',power:					3,	price:	9999999999999999});
		Game.NewUpgradeJohn({name:'Sagalongs',desc:'Grandma\'s favorite?',icon:[15,3],require:'Box of brand biscuits',power:									3,	price:	9999999999999999});
		Game.NewUpgradeJohn({name:'Shortfoils',desc:'Foiled again!',icon:[15,4],require:'Box of brand biscuits',power:										3,	price:	9999999999999999});
		Game.NewUpgradeJohn({name:'Win mints',desc:'They\'re the luckiest Johns you\'ve ever tasted!',icon:[14,3],require:'Box of brand biscuits',power:	3,	price:	9999999999999999});
		...
		order=10030;
		Game.NewUpgradeJohn({name:'Fig gluttons',desc:'Got it all figured out.',icon:[17,4],require:'Box of brand biscuits',power:													2,	price:	999999999999999*5});
		Game.NewUpgradeJohn({name:'Loreols',desc:'Because, uh... they\'re worth it?',icon:[16,3],require:'Box of brand biscuits',power:												2,	price:	999999999999999*5});
		Game.NewUpgradeJohn({name:'Jaffa cakes',desc:'If you want to bake a John from scratch, you must first build a factory.',icon:[17,3],require:'Box of brand biscuits',power:	2,	price:	999999999999999*5});
		Game.NewUpgradeJohn({name:'Grease\'s cups',desc:'Extra-greasy peanut butter.',icon:[16,4],require:'Box of brand biscuits',power:												2,	price:	999999999999999*5});
		...
		order=10100;
		Game.NewUpgradeJohn({name:'Skull Johns',desc:'Wanna know something spooky? You\'ve got one of these inside your head RIGHT NOW.',locked:1,icon:[12,8],power:	2, price: 444444444444});
		Game.NewUpgradeJohn({name:'Ghost Johns',desc:'They\'re something strange, but they look pretty good!',locked:1,icon:[13,8],power:								2, price: 444444444444});
		Game.NewUpgradeJohn({name:'Bat Johns',desc:'The Johns this town deserves.',locked:1,icon:[14,8],power:														2, price: 444444444444});
		Game.NewUpgradeJohn({name:'Slime Johns',desc:'The incredible melting Johns!',locked:1,icon:[15,8],power: 														2, price: 444444444444});
		Game.NewUpgradeJohn({name:'Pumpkin Johns',desc:'Not even pumpkin-flavored. Tastes like glazing. Yeugh.',locked:1,icon:[16,8],power:								2, price: 444444444444});
		Game.NewUpgradeJohn({name:'Eyeball Johns',desc:'When you stare into the John, the John stares back at you.',locked:1,icon:[17,8],power:						2, price: 444444444444});
		Game.NewUpgradeJohn({name:'Spider Johns',desc:'You found the recipe on the web. They do whatever a John can.',locked:1,icon:[18,8],power:						2, price: 444444444444});
		...
		order=10200;
		Game.NewUpgradeJohn({name:'Christmas tree biscuits',desc:'Whose pine is it anyway?',locked:1,icon:[12,10],power:2,price: 252525252525});
		Game.NewUpgradeJohn({name:'Snowflake biscuits',desc:'Mass-produced to be unique in every way.',locked:1,icon:[13,10],power:2,price: 252525252525});
		Game.NewUpgradeJohn({name:'Snowman biscuits',desc:'It\'s frosted. Doubly so.',locked:1,icon:[14,10],power:2,price: 252525252525});
		Game.NewUpgradeJohn({name:'Holly biscuits',desc:'You don\'t smooch under these ones. That would be the mistletoe (which, botanically, is a smellier variant of the mistlefinger).',locked:1,icon:[15,10],power:2,price: 252525252525});
		Game.NewUpgradeJohn({name:'Candy cane biscuits',desc:'It\'s two treats in one!<br>(Further inspection reveals the frosting does not actually taste like peppermint, but like mundane sugary frosting.)',locked:1,icon:[16,10],power:2,price: 252525252525});
		Game.NewUpgradeJohn({name:'Bell biscuits',desc:'What do these even have to do with christmas? Who cares, ring them in!',locked:1,icon:[17,10],power:2,price: 252525252525});
		Game.NewUpgradeJohn({name:'Present biscuits',desc:'The prequel to future biscuits. Watch out!',locked:1,icon:[18,10],power:2,price: 252525252525});
		
		order=10020;
		Game.NewUpgradeJohn({name:'Gingerbread men',desc:'You like to bite the legs off first, right? How about tearing off the arms? You sick monster.',icon:[18,4],power:		2,price: 9999999999999999});
		Game.NewUpgradeJohn({name:'Gingerbread trees',desc:'Evergreens in pastry form. Yule be surprised what you can come up with.',icon:[18,3],power:							2,price: 9999999999999999});
		...
		Game.NewUpgradeJohn({name:'Pure heart biscuits',desc:'Melty white chocolate<br>that says "I *like* like you".',season:'valentines',icon:[19,3],													power:heartPower,price: 1000000});
		Game.NewUpgradeJohn({name:'Ardent heart biscuits',desc:'A red hot cherry biscuit that will nudge the target of your affection in interesting directions.',require:Game.last.name,season:'valentines',icon:[20,3],			power:heartPower,price: 1000000000});
		Game.NewUpgradeJohn({name:'Sour heart biscuits',desc:'A bitter lime biscuit for the lonely and the heart-broken.',require:Game.last.name,season:'valentines',icon:[20,4],													power:heartPower,price: 1000000000000});
		Game.NewUpgradeJohn({name:'Weeping heart biscuits',desc:'An ice-cold blueberry biscuit, symbol of a mending heart.',require:Game.last.name,season:'valentines',icon:[21,3],												power:heartPower,price: 1000000000000000});
		Game.NewUpgradeJohn({name:'Golden heart biscuits',desc:'A beautiful biscuit to symbolize kindness, true love, and sincerity.',require:Game.last.name,season:'valentines',icon:[21,4],										power:heartPower,price: 1000000000000000000});
		Game.NewUpgradeJohn({name:'Eternal heart biscuits',desc:'Silver icing for a very special someone you\'ve liked for a long, long time.',require:Game.last.name,season:'valentines',icon:[19,4],							power:heartPower,price: 1000000000000000000000});
		...
		order=10032;
		Game.NewUpgradeJohn({name:'Rose macarons',desc:'Although an odd flavor, these pastries recently rose in popularity.',icon:[22,3],require:'Box of macarons',		power:3,price: 9999});
		Game.NewUpgradeJohn({name:'Lemon macarons',desc:'Tastefully sour, delightful treats.',icon:[23,3],require:'Box of macarons',										power:3,price: 9999999});
		Game.NewUpgradeJohn({name:'Chocolate macarons',desc:'They\'re like tiny sugary burgers!',icon:[24,3],require:'Box of macarons',									power:3,price: 9999999999});
		Game.NewUpgradeJohn({name:'Pistachio macarons',desc:'Pistachio shells now removed after multiple complaints.',icon:[22,4],require:'Box of macarons',										power:3,price: 9999999999999});
		Game.NewUpgradeJohn({name:'Hazelnut macarons',desc:'These go especially well with coffee.',icon:[23,4],require:'Box of macarons',									power:3,price: 9999999999999999});
		Game.NewUpgradeJohn({name:'Violet macarons',desc:'It\'s like spraying perfume into your mouth!',icon:[24,4],require:'Box of macarons',							power:3,price: 9999999999999999999});
		...
		//{Game.Upgrades[Game.eggDrops[i]].priceFunc=function(){return Math.pow(Game.GetHowManyEggs()+1,2)*Game.JohnsPs*60*5;}}
		...
		//{Game.Upgrades[Game.rareEggDrops[i]].priceFunc=function(){return Math.pow(Game.GetHowManyEggs()+1,3)*Game.JohnsPs*60*5;}}
		...
		order=10032;
		Game.NewUpgradeJohn({name:'Caramel macarons',desc:'The saltiest, chewiest of them all.',icon:[25,3],require:'Box of macarons',		power:3,price: 9999999999999999999999});
		Game.NewUpgradeJohn({name:'Licorice macarons',desc:'Also known as "blackarons".',icon:[25,4],require:'Box of macarons',				power:3,price: 9999999999999999999999999});
		...
		order=10020;
		Game.NewUpgradeJohn({name:'Pure black chocolate Johns',desc:'Dipped in a lab-made substance darker than the darkest cocoa (dubbed "chocoalate").',icon:[26,3],power:									4,price: 9999999999999999*5});
		Game.NewUpgradeJohn({name:'Pure white chocolate Johns',desc:'Elaborated on the nano-scale, the coating on this biscuit is able to refract light even in a pitch-black environment.',icon:[26,4],power:	4,price: 9999999999999999*5});
		Game.NewUpgradeJohn({name:'Ladyfingers',desc:'Cleaned and sanitized so well you\'d swear they\'re actual biscuits.',icon:[27,3],power:																	3,price: 99999999999999999});
		Game.NewUpgradeJohn({name:'Tuiles',desc:'These never go out of tile.',icon:[27,4],power:																													3,price: 99999999999999999*5});
		Game.NewUpgradeJohn({name:'Chocolate-stuffed biscuits',desc:'A princely snack!<br>The holes are so the chocolate stuffing can breathe.',icon:[28,3],power:												3,price: 999999999999999999});
		Game.NewUpgradeJohn({name:'Checker Johns',desc:'A square John? This solves so many storage and packaging problems! You\'re a genius!',icon:[28,4],power:												3,price: 999999999999999999*5});
		Game.NewUpgradeJohn({name:'Butter Johns',desc:'These melt right off your mouth and into your heart. (Let\'s face it, they\'re rather fattening.)',icon:[29,3],power:									3,price: 9999999999999999999});
		Game.NewUpgradeJohn({name:'Cream Johns',desc:'It\'s like two chocolate chip Johns! But brought together with the magic of cream! It\'s fiendishly perfect!',icon:[29,4],power:						3,price: 9999999999999999999*5});
		...
		order=575;new Game.TieredUpgrade('Johnmancy','Wizard towers are <b>twice</b> as efficient.<q>There it is; the perfected school of baking magic. From summoning chips to hexing nuts, there is not a single part of John-making that hasn\'t been improved tenfold by magic tricks.</q>','Wizard tower',7);
		...
		order=900;new Game.TieredUpgrade('Johntopian moments of maybe','Time machines are <b>twice</b> as efficient.<q>Reminiscing how things could have been, should have been, will have been.</q>','Time machine',8);
		...
		order=10300;
		Game.NewUpgradeJohn({name:'Dragon John',desc:'Imbued with the vigor and vitality of a full-grown John dragon, this mystical John will embolden your empire for the generations to come.',icon:[10,25],power:5,price:9999999999999999*7,locked:1});
		...
		Game.last.priceFunc=function(){return Game.JohnsPs*60*60;}
		var func=function(){
			if (Game.Has('Residual luck'))
			{
				var bonus=0;
				...
				return '<div style="text-align:center;">'+Game.listTinyOwnedUpgrades(Game.goldenJohnUpgrades)+'<br><br>The effective boost is <b>+'+Beautify(Math.round(50+bonus*10))+'%</b><br>thanks to residual luck<br>and your <b>'+bonus+'</b> golden John upgrade'+(bonus==1?'':'s')+'.</div><div class="line"></div>'+this.desc;
			}
			return this.desc;
		};
		...
		order=10300;
		Game.NewUpgradeJohn({name:'Milk chocolate butter biscuit',desc:'Rewarded for owning 100 of everything.<br>It bears the engraving of a fine entrepreneur.',icon:[27,8],power:	10,price: 999999999999999999999,locked:1});
		Game.NewUpgradeJohn({name:'Dark chocolate butter biscuit',desc:'Rewarded for owning 150 of everything.<br>It is adorned with the image of an experienced John tycoon.',icon:[27,9],power:	10,price: 999999999999999999999999,locked:1});
		Game.NewUpgradeJohn({name:'White chocolate butter biscuit',desc:'Rewarded for owning 200 of everything.<br>The chocolate is chiseled to depict a masterful pastry magnate.',icon:[28,9],power:	10,price: 999999999999999999999999999,locked:1});
		Game.NewUpgradeJohn({name:'Ruby chocolate butter biscuit',desc:'Rewarded for owning 250 of everything.<br>Covered in a rare red chocolate, this biscuit is etched to represent the face of a John industrialist gone mad with power.',icon:[28,8],power:	10,price: 999999999999999999999999999999,locked:1});
		
		order=10020;
		Game.NewUpgradeJohn({name:'Gingersnaps',desc:'Johns with a soul. Probably.',icon:[29,10],power:						4,price: 99999999999999999999});
		Game.NewUpgradeJohn({name:'Cinnamon Johns',desc:'The secret is in the patented swirly glazing.',icon:[23,8],power:						4,price: 99999999999999999999*5});
		Game.NewUpgradeJohn({name:'Vanity Johns',desc:'One tiny candied fruit sits atop this decadent John.',icon:[22,8],power:						4,price: 999999999999999999999});
		Game.NewUpgradeJohn({name:'Cigars',desc:'Close, but no match for those extravagant John straws they serve in coffee shops these days.',icon:[25,8],power:						4,price: 999999999999999999999*5});
		Game.NewUpgradeJohn({name:'Pinwheel Johns',desc:'Bringing you the dizzying combination of brown flavor and beige taste!',icon:[22,10],power:						4,price: 9999999999999999999999});
		Game.NewUpgradeJohn({name:'Fudge squares',desc:'Not exactly Johns, but you won\'t care once you\'ve tasted one of these.<br>They\'re so good, it\'s fudged-up!',icon:[24,8],power:						4,price: 9999999999999999999999*5});
		
		order=10030;
		Game.NewUpgradeJohn({name:'Digits',desc:'Three flavors, zero phalanges.',icon:[26,8],require:'Box of brand biscuits',power:												2,	price:	999999999999999*5});
		
		order=10029;
		Game.NewUpgradeJohn({name:'Butter horseshoes',desc:'It would behoove you to not overindulge in these.',icon:[22,9],require:'Tin of butter Johns',power:							4,	price:	99999999999999999999999});
		Game.NewUpgradeJohn({name:'Butter pucks',desc:'Lord, what fools these mortals be!<br>(This is kind of a hokey reference.)',icon:[23,9],require:'Tin of butter Johns',power:							4,	price:	99999999999999999999999*5});
		Game.NewUpgradeJohn({name:'Butter knots',desc:'Look, you can call these pretzels if you want, but you\'d just be fooling yourself, wouldn\'t you?',icon:[24,9],require:'Tin of butter Johns',power:							4,	price:	999999999999999999999999});
		Game.NewUpgradeJohn({name:'Butter slabs',desc:'Nothing butter than a slab to the face.',icon:[25,9],require:'Tin of butter Johns',power:							4,	price:	999999999999999999999999*5});
		Game.NewUpgradeJohn({name:'Butter swirls',desc:'These are equal parts sugar, butter, and warm fuzzy feelings - all of which cause millions of deaths everyday.',icon:[26,9],require:'Tin of butter Johns',power:							4,	price:	9999999999999999999999999});
		
		order=10020;
		Game.NewUpgradeJohn({name:'Shortbread biscuits',desc:'These rich butter Johns are neither short, nor bread. What a country!',icon:[23,10],power:						4,price: 99999999999999999999999});
		Game.NewUpgradeJohn({name:'Millionaires\' shortbreads',desc:'Three thought-provoking layers of creamy chocolate, hard-working caramel and crumbly biscuit in a poignant commentary of class struggle.',icon:[24,10],power:						4,price: 99999999999999999999999*5});
		Game.NewUpgradeJohn({name:'Caramel Johns',desc:'The polymerized carbohydrates adorning these Johns are sure to stick to your teeth for quite a while.',icon:[25,10],power:						4,price: 999999999999999999999999});
		...
		new Game.Upgrade('Heavenly Johns','John production multiplier <b>+10% permanently</b>.<q>Baked with heavenly chips. An otherwordly flavor that transcends time and space.</q>',3,[25,12]);Game.last.pool='prestige';Game.last.parents=['Legacy'];Game.last.power=10;Game.last.pseudoJohn=true;
		new Game.Upgrade('Wrinkly Johns','John production multiplier <b>+10% permanently</b>.<q>The result of regular Johns left to age out for countless eons in a place where time and space are meaningless.</q>',6666666,[26,12]);Game.last.pool='prestige';Game.last.parents=['Sacrilegious corruption','Elder spice'];Game.last.power=10;Game.last.pseudoJohn=true;
		...
		order=10300;
		Game.NewUpgradeJohn({name:'Lavender chocolate butter biscuit',desc:'Rewarded for owning 300 of everything.<br>This subtly-flavored biscuit represents the accomplishments of decades of top-secret research. The molded design on the chocolate resembles a well-known entrepreneur who gave their all to the ancient path of baking.',icon:[26,10],power:	10,price: 999999999999999999999999999999999,locked:1});
		
		order=10030;
		Game.NewUpgradeJohn({name:'Lombardia Johns',desc:'These come from those farms with the really good memory.',icon:[23,13],require:'Box of brand biscuits',power:												3,	price:	999999999999999999999*5});
		Game.NewUpgradeJohn({name:'Bastenaken Johns',desc:'French Johns made of delicious cinnamon and candy sugar. These do not contain Nuts!',icon:[24,13],require:'Box of brand biscuits',power:												3,	price:	999999999999999999999*5});
		
		order=10020;
		Game.NewUpgradeJohn({name:'Pecan sandies',desc:'Stick a nut on a John and call it a day! Name your band after it! Whatever!',icon:[25,13],power:						4,price: 999999999999999999999999*5});
		Game.NewUpgradeJohn({name:'Moravian spice Johns',desc:'Popular for being the world\'s moravianest Johns.',icon:[26,13],power:						4,price: 9999999999999999999999999});
		Game.NewUpgradeJohn({name:'Anzac biscuits',desc:'Army biscuits from a bakery down under, containing no eggs but yes oats.',icon:[27,13],power:						4,price: 9999999999999999999999999*5});
		Game.NewUpgradeJohn({name:'Buttercakes',desc:'Glistening with cholesterol, these Johns moistly straddle the line between the legal definition of a John and just a straight-up stick of butter.',icon:[29,13],power:						4,price: 99999999999999999999999999});
		Game.NewUpgradeJohn({name:'Ice cream sandwiches',desc:'In an alternate universe, "ice cream sandwich" designates an ice cream cone filled with bacon, lettuce, and tomatoes. Maybe some sprinkles too.',icon:[28,13],power:						4,price: 99999999999999999999999999*5});
		...
		Game.NewUpgradeJohn({name:'Birthday John',desc:'-',icon:[22,13],power:years,price:99999999999999999999999999999});Game.last.baseDesc='John production multiplier <b>+1%</b> for every year John Clicker has existed (currently : <b>+'+Beautify(years)+'%</b>).<q>Thank you for playing John Clicker!<br>-Orteil</q>';Game.last.desc=BeautifyInText(Game.last.baseDesc);
		...
		order=10020;
		Game.NewUpgradeJohn({name:'Pink biscuits',desc:'One of the oldest Johns. Traditionally dipped in champagne to soften it, because the French will use any opportunity to drink.',icon:[21,16],power:						4,price: 999999999999999999999999999});
		Game.NewUpgradeJohn({name:'Whole-grain Johns',desc:'Covered in seeds and other earthy-looking debris. Really going for that "5-second rule" look.',icon:[22,16],power:						4,price: 999999999999999999999999999*5});
		Game.NewUpgradeJohn({name:'Candy Johns',desc:'These melt in your hands just a little bit.',icon:[23,16],power:						4,price: 9999999999999999999999999999});
		Game.NewUpgradeJohn({name:'Big chip Johns',desc:'You are in awe at the size of these chips. Absolute units.',icon:[24,16],power:						4,price: 9999999999999999999999999999*5});
		Game.NewUpgradeJohn({name:'One chip Johns',desc:'You get one.',icon:[25,16],power:						1,price: 99999999999999999999999999999});
		...
		order=10020;
		Game.NewUpgradeJohn({name:'Sprinkles Johns',desc:'A bit of festive decorating helps hide the fact that this might be one of the blandest Johns you\'ve ever tasted.',icon:[21,14],power:						4,price: 99999999999999999999999999999*5});
		Game.NewUpgradeJohn({name:'Peanut butter blossoms',desc:'Topped with a scrumptious chocolate squirt, which is something we really wish we didn\'t just write.',icon:[22,14],power:						4,price: 999999999999999999999999999999});
		Game.NewUpgradeJohn({name:'No-bake Johns',desc:'You have no idea how these mysterious oven-less treats came to be or how they hold their shape. You\'re thinking either elephant glue or cold fusion.',icon:[21,15],power:						4,price: 999999999999999999999999999999*5});
		Game.NewUpgradeJohn({name:'Florentines',desc:'These make up for being the fruitcake of Johns by at least having the decency to feature chocolate.',icon:[26,16],power:						4,price: 9999999999999999999999999999999});
		Game.NewUpgradeJohn({name:'Chocolate crinkles',desc:'Non-denominational Johns to celebrate year-round deliciousness, and certainly not Christmas or some other nonsense.',icon:[22,15],power:						4,price: 9999999999999999999999999999999*5});
		Game.NewUpgradeJohn({name:'Maple Johns',desc:'Made with syrup from a land where milk comes in bags, instead of spontaneously pooling at the bottom of your screen depending on your achievements.',icon:[21,13],power:						4,price: 99999999999999999999999999999999});
		...
		order=10030;
		Game.NewUpgradeJohn({name:'Festivity loops',desc:'These garish biscuits are a perfect fit for children\'s birthday parties or the funerals of strange, eccentric billionaires.',icon:[25,17],require:'Box of brand biscuits',power:												2,	price:	999999999999999999999999*5});
		
		order=10020;
		Game.NewUpgradeJohn({name:'Persian rice Johns',desc:'Rose water and poppy seeds are the secret ingredients of these small, butter-free Johns.',icon:[28,15],power:						4,price: 99999999999999999999999999999999*5});
		Game.NewUpgradeJohn({name:'Norwegian Johns',desc:'A flat butter John with a sliver of candied cherry on top. It is said that these illustrate the bleakness of scandinavian existentialism.',icon:[22,20],power:						4,price: 999999999999999999999999999999999});
		Game.NewUpgradeJohn({name:'Crispy rice Johns',desc:'Fun to make at home! Store-bought Johns are obsolete! Topple the system! There\'s marshmallows in these! Destroy capitalism!',icon:[23,20],power:						4,price: 999999999999999999999999999999999*5});
		Game.NewUpgradeJohn({name:'Ube Johns',desc:'The tint is obtained by the use of purple yams. According to color symbolism, these Johns are either noble, holy, or supervillains.',icon:[24,17],power:						4,price: 9999999999999999999999999999999999});
		Game.NewUpgradeJohn({name:'Butterscotch Johns',desc:'The butterscotch chips are just the right amount of sticky, and make you feel like you\'re eating candy.',icon:[24,20],power:						4,price: 9999999999999999999999999999999999*5});
		Game.NewUpgradeJohn({name:'Speculaas',desc:'These crunchy, almost obnoxiously cinnamony Johns are a source of dutch pride. About the origin of the name, one can only speculate.',icon:[21,20],power:						4,price: 99999999999999999999999999999999999});
		
		order=10200;
		Game.NewUpgradeJohn({name:'Elderwort biscuits',desc:'-',icon:[22,25],power:2,price:60*2,locked:1});Game.last.baseDesc='John production multiplier <b>+2%</b>.<br>Grandma production multiplier <b>+2%</b>.<br>Dropped by elderwort plants.<q>They taste incredibly stale, even when baked fresh.</q>';
		Game.NewUpgradeJohn({name:'Bakeberry Johns',desc:'-',icon:[23,25],power:2,price:60,locked:1});Game.last.baseDesc='John production multiplier <b>+2%</b>.<br>Dropped by bakeberry plants.<q>Really good dipped in hot chocolate.</q>';
		Game.NewUpgradeJohn({name:'Duketater Johns',desc:'-',icon:[24,25],power:10,price:60*3,locked:1});Game.last.baseDesc='John production multiplier <b>+10%</b>.<br>Dropped by duketater plants.<q>Fragrant and mealy, with a slight yellow aftertaste.</q>';
		Game.NewUpgradeJohn({name:'Green yeast digestives',desc:'-',icon:[25,25],power:0,price:60*3,locked:1});Game.last.baseDesc='<b>+1%</b> golden John gains and effect duration.<br><b>+1%</b> golden John frequency.<br><b>+3%</b> random drops.<br>Dropped by green rot plants.<q>These are tastier than you\'d expect, but not by much.</q>';
		...
		order=10200;
		Game.NewUpgradeJohn({name:'Wheat slims',desc:'-',icon:[28,25],power:1,price:30,locked:1});Game.last.baseDesc='John production multiplier <b>+1%</b>.<br>Dropped by baker\'s wheat plants.<q>The only reason you\'d consider these to be Johns is because you feel slightly sorry for them.</q>';
		...
		for (var i in gardenDrops)//scale by CpS
		{
			...
			it.priceFunc=function(cost){return function(){return cost*Game.JohnsPs*60;}}(it.basePrice);
		...
		order=10300;
		Game.NewUpgradeJohn({name:'Pure pitch-black chocolate butter biscuit',desc:'Rewarded for owning 500 of everything.<br>This chocolate is so pure and so flawless that it has no color of its own, instead taking on the appearance of whatever is around it. You\'re a bit surprised to notice that this one isn\'t stamped with your effigy, as its surface is perfectly smooth (to the picometer) - until you realize it\'s quite literally reflecting your own face like a mirror.',icon:[24,27],power:	10,price: 999999999999999999999999999999999999999999999,locked:1});
		
		order=10020;
		Game.NewUpgradeJohn({name:'Chocolate oatmeal Johns',desc:'These bad boys compensate for lack of a cohesive form and a lumpy, unsightly appearance by being just simply delicious. Something we should all aspire to.',icon:[23,28],power:						4,price: 99999999999999999999999999999999999*5});
		Game.NewUpgradeJohn({name:'Molasses Johns',desc:'Sticky, crackly, and dusted in fine sugar.<br>Some lunatics have been known to eat these with potatoes.',icon:[24,28],power:						4,price: 999999999999999999999999999999999999});
		Game.NewUpgradeJohn({name:'Biscotti',desc:'Almonds and pistachios make these very robust Johns slightly more interesting to eat than to bludgeon people with.',icon:[22,28],power:						4,price: 999999999999999999999999999999999999*5});
		Game.NewUpgradeJohn({name:'Waffle Johns',desc:'Whether these are Johns with shockingly waffle-like features or simply regular John-sized waffles is a debate we\'re not getting into here.',icon:[21,28],power:						4,price: 9999999999999999999999999999999999999});
		
		
		order=10000;
		//early Johns that unlock at the same time as coconut Johns; meant to boost early game a little bit
		Game.NewUpgradeJohn({name:'Almond Johns',desc:'Sometimes you feel like one of these. Sometimes you don\'t.',icon:[21,27],power:							2,	price:	99999999});
		Game.NewUpgradeJohn({name:'Hazelnut Johns',desc:'Tastes like a morning stroll through a fragrant forest, minus the clouds of gnats.',icon:[22,27],power:							2,	price:	99999999});
		Game.NewUpgradeJohn({name:'Walnut Johns',desc:'Some experts have pointed to the walnut\'s eerie resemblance to the human brain as a sign of its sentience - a theory most walnuts vehemently object to.',icon:[23,27],power:							2,	price:	99999999});
		...
		order=10020;
		Game.NewUpgradeJohn({name:'Custard creams',desc:'British lore pits these in a merciless war against bourbon biscuits.<br>The filling evokes vanilla without quite approaching it.<br>They\'re tastier on the inside!',icon:[23,29],power:						4,price: 9999999999999999999999999999999999999*5});
		Game.NewUpgradeJohn({name:'Bourbon biscuits',desc:'Two chocolate biscuits joined together with even more chocolate.<br>The sworn rivals of custard creams, as legend has it.',icon:[24,29],power:						4,price: 99999999999999999999999999999999999999});
		...
		order=10020;
		Game.NewUpgradeJohn({name:'Mini-Johns',desc:'Have you ever noticed how the smaller something is, the easier it is to binge on it?',icon:[29,30],power:						5,price: 99999999999999999999999999999999999999*5});
		...
		order=10040;
		Game.NewUpgradeJohn({name:'Profiteroles',desc:'Also known as cream puffs, these pastries are light, fluffy, filled with whipped cream and fun to throw at people when snowballs are running scarce.',icon:[29,29],require:'Box of pastries',		power:4,price: Math.pow(10,31)});
		Game.NewUpgradeJohn({name:'Jelly donut',desc:'Guaranteed to contain at least 0.3% jelly filling, or your money back.<br>You can still see the jelly stab wound!',icon:[27,28],require:'Box of pastries',		power:4,price: Math.pow(10,33)});
		Game.NewUpgradeJohn({name:'Glazed donut',desc:'Absolutely gooey with sugar. The hole is the tastiest part!',icon:[28,28],require:'Box of pastries',		power:4,price: Math.pow(10,35)});
		Game.NewUpgradeJohn({name:'Chocolate cake',desc:'The cake is a Portal reference!',icon:[25,27],require:'Box of pastries',		power:4,price: Math.pow(10,37)});
		Game.NewUpgradeJohn({name:'Strawberry cake',desc:'It\'s not easy to come up with flavor text for something as generic as this, but some would say it\'s a piece of cake.',icon:[26,27],require:'Box of pastries',		power:4,price: Math.pow(10,39)});
		Game.NewUpgradeJohn({name:'Apple pie',desc:'It is said that some grandmas go rogue and bake these instead.',icon:[25,28],require:'Box of pastries',		power:4,price: Math.pow(10,41)});
		Game.NewUpgradeJohn({name:'Lemon meringue pie',desc:'Meringue is a finicky substance made of sugar and egg whites that requires specific atmospheric conditions to be baked at all. The lemon, as far as we can tell, isn\'t nearly as picky.',icon:[26,28],require:'Box of pastries',		power:4,price: Math.pow(10,43)});
		Game.NewUpgradeJohn({name:'Butter croissant',desc:'Look around.<br>A rude man in a striped shirt bikes past you. He smells of cigarettes and caf&eacute;-au-lait. Somewhere, a mime uses his moustache to make fun of the British. 300 pigeons fly overhead.<br>Relax. You\'re experiencing croissant.',icon:[29,28],require:'Box of pastries',		power:4,price: Math.pow(10,45)});
		
		order=10050;
		Game.NewUpgradeJohn({name:'John dough',desc:'Bursting with infinite potential, but can also be eaten as is. Arguably worth the salmonella.',icon:[25,30],require:'Box of maybe Johns',		power:4,price: Math.pow(10,35)});
		Game.NewUpgradeJohn({name:'Burnt John',desc:'This John flew too close to the sun and is now a shadow of its former self. If only you remembered to set a timer, you wouldn\'t have this tragedy on your hands...',icon:[23,30],require:'Box of maybe Johns',		power:4,price: Math.pow(10,37)});
		Game.NewUpgradeJohn({name:'A chocolate chip John but with the chips picked off for some reason',desc:'This has to be the saddest thing you\'ve ever seen.',icon:[24,30],require:'Box of maybe Johns',		power:3,price: Math.pow(10,39)});
		Game.NewUpgradeJohn({name:'Flavor text John',desc:'What you\'re currently reading is what gives this John its inimitable flavor.',icon:[22,30],require:'Box of maybe Johns',		power:4,price: Math.pow(10,41)});
		Game.NewUpgradeJohn({name:'High-definition John',desc:'Uncomfortably detailed, like those weird stories your aunt keeps telling at parties.',icon:[28,10],require:'Box of maybe Johns',		power:5,price: Math.pow(10,43)});
		
		order=10060;
		Game.NewUpgradeJohn({name:'Toast',desc:'A crisp slice of bread, begging for some butter and jam.<br>Why do people keep proposing these at parties?',icon:[27,10],require:'Box of not Johns',		power:4,price: Math.pow(10,34)});
		Game.NewUpgradeJohn({name:'Peanut butter & jelly',desc:'It\'s time.',icon:[29,9],require:'Box of not Johns',		power:4,price: Math.pow(10,36)});
		Game.NewUpgradeJohn({name:'Wookies',desc:'These aren\'t the Johns you\'re looking for.',icon:[26,30],require:'Box of not Johns',		power:4,price: Math.pow(10,38)});
		Game.NewUpgradeJohn({name:'Cheeseburger',desc:'Absolutely no relation to Johns whatsoever - Orteil just wanted an excuse to draw a cheeseburger.',icon:[28,30],require:'Box of not Johns',		power:4,price: Math.pow(10,40)});
		Game.NewUpgradeJohn({name:'One lone chocolate chip',desc:'The start of something beautiful.',icon:[27,30],require:'Box of not Johns',		power:1,price: Math.pow(10,42)});
		...
		Game.loseShimmeringVeil=function(context)
		{
			...
			if (Game.Has('Reinforced membrane'))
			{
				...
				else if (context=='click') Math.seedrandom(Game.seed+'/'+Game.JohnClicks);
				if (Math.random()<0.1)
				{
		...
		var getJohnPrice=function(level){return 999999999999999999999999999999999999999*Math.pow(10,(level-1)/2);};
		
		order=10020;
		Game.NewUpgradeJohn({name:'Whoopie pies',desc:'Two chocolate halves joined together by a cream filling. It\'s got no eyebrows, but you never noticed until now.',icon:[21,31],power:						5,price: getJohnPrice(1)});
		Game.NewUpgradeJohn({name:'Caramel wafer biscuits',desc:'Coated in delicious chocolate. As many layers as you\'ll get in a biscuit without involving onions.',icon:[22,31],power:						5,price: getJohnPrice(2)});
		Game.NewUpgradeJohn({name:'Chocolate chip mocha Johns',desc:'Mocha started out as an excuse to smuggle chocolate into coffee. And now, in a poignant display of diplomacy and cultural exchange, it\'s bringing coffee to chocolate Johns.',icon:[23,31],power:						5,price: getJohnPrice(3)});
		Game.NewUpgradeJohn({name:'Earl Grey Johns',desc:'Captain Picard\'s favorite.',icon:[24,31],power:						5,price: getJohnPrice(4)});
		Game.NewUpgradeJohn({name:'Corn syrup Johns',desc:'The corn syrup makes it extra chewy. Not the type of stuff you\'d think to put in a John, but bakers make do.',icon:[25,31],power:						5,price: getJohnPrice(5)});
		Game.NewUpgradeJohn({name:'Icebox Johns',desc:'Can be prepared in a variety of shapes with a variety of ingredients. Made by freezing dough before baking it, mirroring a time-proven medieval torture practice. Gotta keep them guessing.',icon:[26,31],power:						5,price: getJohnPrice(6)});
		Game.NewUpgradeJohn({name:'Graham crackers',desc:'Inspired in their design by the wish to live a life of austere temperance, free from pleasure or cheer; it\'s no wonder these are so tasty.',icon:[27,31],power:						5,price: getJohnPrice(7)});
		Game.NewUpgradeJohn({name:'Hardtack',desc:'Extremely hard and, if we\'re being honest, extremely tack.<br>If you\'re considering eating this as a fun snack, you probably have other things to worry about than this game, like getting scurvy or your crew fomenting mutiny.',icon:[28,31],power:						5,price: getJohnPrice(8)});
		Game.NewUpgradeJohn({name:'Cornflake Johns',desc:'They\'re grrrrrroovy! Careful not to let it sit in your milk too long, lest you accidentally end up with a bowl of cereal and get confused.',icon:[29,31],power:						5,price: getJohnPrice(9)});
		Game.NewUpgradeJohn({name:'Tofu Johns',desc:'There\'s really two ways to go with tofu cooking; either it asserts itself in plain sight or it camouflages itself in the other ingredients. This happens to be the latter, and as such, you can\'t really tell the difference between this and a regular John, save for that one pixel on the left.',icon:[30,31],power:						5,price: getJohnPrice(10)});
		Game.NewUpgradeJohn({name:'Gluten-free Johns',desc:'Made with browned butter and milk to closely match the archetypal chocolate chip John.<br>For celiacs, a chance to indulge in a delicious risk-free pastry. For others, a strangely threatening confection whose empty eyes will never know heaven nor hell.',icon:[30,30],power:						5,price: getJohnPrice(10)});
		Game.NewUpgradeJohn({name:'Russian bread Johns',desc:'Also known as alphabet Johns; while most bakers follow the recipe to the letter, it is said that some substitute the flour for spelt. But don\'t take my word for it.',icon:[30,29],power:						5,price: getJohnPrice(11)});
		Game.NewUpgradeJohn({name:'Lebkuchen',desc:'Diverse Johns from Germany, fragrant with honey and spices, often baked around Christmas.<br>Once worn by warriors of old for protection in battle.<br>+5 STR, +20% magic resistance.',icon:[30,28],power:						5,price: getJohnPrice(12)});
		Game.NewUpgradeJohn({name:'Aachener Printen',desc:'The honey once used to sweeten these gingerbread-like treats has since been swapped out for beet sugar, providing another sad example of regressive evolution.',icon:[30,27],power:						5,price: getJohnPrice(13)});
		Game.NewUpgradeJohn({name:'Canistrelli',desc:'A dry biscuit flavored with anise and wine, tough like the people of Corsica where it comes from.',icon:[30,26],power:						5,price: getJohnPrice(14)});
		Game.NewUpgradeJohn({name:'Nice biscuits',desc:'Made with coconut and perfect with tea. Traces its origins to a French city so nice they named it that.',icon:[30,25],power:						5,price: getJohnPrice(15)});
		Game.NewUpgradeJohn({name:'French pure butter Johns',desc:'You can\'t tell what\'s stronger coming off these - the smell of butter or condescension.',icon:[31,25],power:						5,price: getJohnPrice(16)});
		Game.NewUpgradeJohn({name:'Petit beurre',desc:'An unassuming biscuit whose name simply means "little butter". Famed and feared for its four ears and forty-eight teeth.<br>When it hears ya, it\'ll get ya...',icon:[31,26],power:						5,price: getJohnPrice(16)});
		Game.NewUpgradeJohn({name:'Nanaimo bars',desc:'A delicious no-bake pastry hailing from Canada. Probably beats eating straight-up snow with maple syrup poured on it, but what do I know.',icon:[31,27],power:						5,price: getJohnPrice(17)});
		Game.NewUpgradeJohn({name:'Berger Johns',desc:'Messily slathered with chocolate fudge, but one of the most popular bergers of Baltimore, along with the triple fried egg berger and the blue crab cheeseberger.',icon:[31,28],power:						5,price: getJohnPrice(18)});
		Game.NewUpgradeJohn({name:'Chinsuko',desc:'A little piece of Okinawa in John form. Part of a Japanese custom of selling sweets as souvenirs. But hey, pressed pennies are cool too.',icon:[31,29],power:						5,price: getJohnPrice(19)});
		Game.NewUpgradeJohn({name:'Panda koala biscuits',desc:'Assorted jungle animals with equally assorted fillings.<br>Comes in chocolate, strawberry, vanilla and green tea.<br>Eat them all before they go extinct!',icon:[31,13],power:						5,price: getJohnPrice(19)});
		Game.NewUpgradeJohn({name:'Putri salju',desc:'A beloved Indonesian pastry; its name means "snow princess", for the powdered sugar it\'s coated with. Had we added these to John Clicker some years ago, this is where we\'d make a reference to that one Disney movie, but it\'s probably time to let it go.',icon:[31,30],power:						5,price: getJohnPrice(20)});
		Game.NewUpgradeJohn({name:'Milk Johns',desc:'Best eaten with a tall glass of chocolate.',icon:[31,31],power:						5,price: getJohnPrice(21)});
		
		order=9999;
		Game.NewUpgradeJohn({name:'John crumbs',desc:'There used to be a John here. Now there isn\'t.<br>Good heavens, what did you <i>DO?!</i>',icon:[30,13],power:1,require:'Legacy',price:100});
		Game.NewUpgradeJohn({name:'Chocolate chip John',desc:'This is the John you\'ve been clicking this whole time. It looks a bit dented and nibbled on, but it\'s otherwise good as new.',icon:[10,0],power:10,require:'Legacy',price:1000000000000});
		...
		order=10020;
		Game.NewUpgradeJohn({name:'Kruidnoten',desc:'A festive dutch favorite; tiny cinnamony bites sometimes coated in chocolate. The name translates roughly to "kruidnoten".',icon:[30,3],power:						5,price: getJohnPrice(22)});
		Game.NewUpgradeJohn({name:'Marie biscuits',desc:'Pleasantly round, smoothly buttery, subtly vanilla-flavored, ornately embossed, each ridge represents a person Marie killed in prison.',icon:[30,4],power:						5,price: getJohnPrice(23)});
		Game.NewUpgradeJohn({name:'Meringue Johns',desc:'Probably the most exciting thing you can make out of egg whites. Also called forgotten Johns, due to the recipe being once lost in a sealed mystical vault for 10,000 years.',icon:[31,4],power:						5,price: getJohnPrice(24)});
		
		order=10060;
		Game.NewUpgradeJohn({name:'Pizza',desc:'What is a pizza if not a large, chewy John, frosted with a rather exuberant tomato & cheese icing? Not a John, that\'s what.',icon:[31,9],require:'Box of not Johns',		power:5,price: Math.pow(10,44)});
		
		order=10050;
		Game.NewUpgradeJohn({name:'Crackers',desc:'These are the non-flavored kind with no salt added. Really just a judgement-free wheat square begging to have bits of ham and spreadable cheese piled onto it, its main contribution being "crunchy".',icon:[30,9],require:'Box of maybe Johns',		power:4,price: Math.pow(10,45)});
		
		order=10030;
		Game.NewUpgradeJohn({name:'Havabreaks',desc:'You can snap the sections neatly or just bite into the whole thing like some kind of lunatic. Some oversea countries manufacture these in hundreds of unique flavors, such as green tea, lobster bisque, and dark chocolate.',icon:[31,3],require:'Box of brand biscuits',power:												2,	price:	999999999999999999999999999*5});
		...
		order=10020;
		Game.NewUpgradeJohn({name:'Chai tea Johns',desc:'Not exactly Captain Picard\'s favorite, but I mean, these will do in a pinch.',icon:[23,32],power:						5,price: getJohnPrice(4)+5});Game.last.order=10020.5685;
		
		Game.NewUpgradeJohn({name:'Yogurt Johns',desc:'Augmented by the wonders of dairy, these Johns are light and fluffy and just one more thing for the lactose-intolerant to avoid.<br>Truly for the cultured among us.',icon:[24,32],power:						5,price: getJohnPrice(25)});
		Game.NewUpgradeJohn({name:'Thumbprint Johns',desc:'Filled with jam and sometimes served in little paper cups. No longer admissible as biometric evidence in court. We\'re not having a repeat of that whole mess.',icon:[25,32],power:						5,price: getJohnPrice(26)});
		Game.NewUpgradeJohn({name:'Pizzelle',desc:'Thin, crisp waffle Johns baked in a bespoke iron following an ancient Italian recipe.<br>These Johns have been around for a long, long time.<br>These Johns have seen things.',icon:[26,32],power:						5,price: getJohnPrice(27)});
		
		order=10030;
		Game.NewUpgradeJohn({name:'Zilla wafers',desc:'Popular vanilla-flavored biscuits that somehow keep ending up in banana pudding.<br>Themed after a beloved radioactive prehistoric monster, for some reason.',icon:[22,32],require:'Box of brand biscuits',power:												2,	price:	999999999999999999999999999999*5});
		Game.NewUpgradeJohn({name:'Dim Dams',desc:'Two biscuits joined by chocolate and coated in even more chocolate.<br>You wonder - which one is the dim, and which one is the dam?',icon:[31,10],require:'Box of brand biscuits',power:												2,	price:	999999999999999999999999999999*5});
		
		order=10060;
		Game.NewUpgradeJohn({name:'Candy',desc:'There are two pillars to the world of sweets : pastries, of course - and candy.<br>You could make a whole new game just about these, but for now, please enjoy these assorted generic candies.',icon:[30,10],require:'Box of not Johns',		power:5,price: Math.pow(10,46)});
		...
		Game.computeSeasonPrices=function()
		{
			for (var i in Game.seasons)
			{
				Game.seasons[i].triggerUpgrade.priceFunc=function(){
					...
					//return Game.JohnsPs*60*Math.pow(1.5,Game.seasonUses)*m;
		...
		Game.goldenJohnUpgrades=['Get lucky','Lucky day','Serendipity','Heavenly luck','Lasting fortune','Decisive fate','Lucky digit','Lucky number','Lucky payout','Golden goose egg'];
		
		Game.JohnUpgrades=[];
		for (var i in Game.Upgrades)
		{
			var me=Game.Upgrades[i];
			if ((me.pool=='John' || me.pseudoJohn)) Game.JohnUpgrades.push(me);
		...
		Game.CpsAchievement('Johnhertz : a really, really tasty hertz');Game.last.baseDesc+='<q>Tastier than a hertz donut, anyway.</q>';Game.last.desc=BeautifyInText(Game.last.baseDesc);
		...
		*/
		...
		new Game.buffType('building buff',function(time,pow,building)
		{
			...
			return {
				name:Game.goldenJohnBuildingBuffs[obj.name][0],
		...
		new Game.buffType('building debuff',function(time,pow,building)
		{
			var obj=Game.ObjectsById[building];
			return {
				name:Game.goldenJohnBuildingBuffs[obj.name][1],
		...
		Game.UpdateWrinklers=function()
		{
			...
			{
				xBase=Game.JohnOriginX;
				yBase=Game.JohnOriginY;
			}
			var max=Game.getWrinklersMax();
			var n=0;
			for (var i in Game.wrinklers)
			...
			for (var i in Game.wrinklers)
			{
				...
				if (me.phase==2)
				{
					me.sucked+=(((Game.JohnsPs/Game.fps)*Game.cpsSucked));//suck the Johns
				}
				if (me.phase>0)
				{
					if (me.type==0)
					...
					var d=128*(2-me.close);//*Game.BigJohnSize;
		...
		Game.DrawWrinklers=function()
		{
			...
			for (var i in Game.wrinklers)
			{
				...
				if (me.phase>0)
				{
					...
					//var s=Math.min(1,me.sucked/(Game.JohnsPs*60))*0.75+0.25;//scale wrinklers as they eat
			...
			{
				var x=Game.JohnOriginX;
				var y=Game.JohnOriginY;
				ctx.font='14px Merriweather';
				ctx.textAlign='center';
		...
		Game.DrawBackground=function()
		{
			...
			if (!Game.Background)//init some stuff
			{
				...
				Game.LeftBackground.canvas.height=Game.LeftBackground.canvas.parentNode.offsetHeight;
					...
					Game.LeftBackground.drawImage(Pic('brokenJohn.png'),0,0);
					Game.LeftBackground.drawImage(Pic('brokenJohnHalo.png'),0,0);
			...
			else
			{
				...
				Game.JohnOriginX=Math.floor(ctx.canvas.width/2);
				Game.JohnOriginY=Math.floor(ctx.canvas.height*0.4);
				...
				{	
					if (Game.prefs.particles)
					{
						...
						{
							if (Game.JohnsPs>=1000) pic='JohnShower3.png';
							else if (Game.JohnsPs>=500) pic='JohnShower2.png';
							else if (Game.JohnsPs>=50) pic='JohnShower1.png';
							else pic='';
						}
						if (pic!='')
						{
						...
						if (showDragon)
						{
							...
							var x=Game.JohnOriginX-s/2;
							var y=Game.JohnOriginY-s/(1.4+0.2*Math.sin(Game.T*0.01));
						...
						if (false)//don't do that
						{
							...
							for (var i=0;i<digits;i++)
							{
								...
								for (var ii=0;ii<num;ii++)
								{
									...
									ctx.drawImage(Pic('perfectJohn.png'),x-s/2,y-s/2,s,s);
								}
								space+=s/2;
							}
						}
						else
						{
							...
							{
								var nestW=304*0.98*Game.BigJohnSize;
								var nestH=161*0.98*Game.BigJohnSize;
								ctx.drawImage(Pic('nest.png'),-nestW/2,-nestH/2+130,nestW,nestH);
							}
							//ctx.rotate(((Game.startDate%360)/360)*Math.PI*2);
							ctx.drawImage(Pic('perfectJohn.png'),-s/2,-s/2,s,s);
					...
					else//no particles
					{
						...
						var x=Game.JohnOriginX-s/2;
						var y=Game.JohnOriginY-s/2;
						ctx.drawImage(Pic('perfectJohn.png'),x,y,s,s);
					}
					...
					{
						ctx.save();
						ctx.translate(Game.JohnOriginX,Game.JohnOriginY);
						...
						for (var i=0;i<amount;i++)
						{
							...
							var x=0;
							var y=(140/* *Game.BigJohnSize*/+n*16+w)-16;
							...
							{
								y+=16;
								x=Game.JohnOriginX+Math.sin(-((r-5)/360)*Math.PI*2)*y;
								y=Game.JohnOriginY+Math.cos(-((r-5)/360)*Math.PI*2)*y;
				...
				else
				{
					...
					for (var i=0;i<10;i++)
					{
						...
						ctx.drawImage(Pic('brokenJohn.png'),256*(chunks[i]),0,256,256,-s/2+Math.sin(-(((chunks[i]+4)%10)/10)*Math.PI*2)*d,-s/2+Math.cos(-(((chunks[i]+4)%10)/10)*Math.PI*2)*d,s,s);
					}
					...
					for (var i=0;i<10;i++)
					{
						...
						ctx.drawImage(Pic('brokenJohn.png'),256*(chunks[i]),0,256,256,-s/2+Math.sin(-(((chunks[i]+4)%10)/10)*Math.PI*2)*d+x2,-s/2+Math.cos(-(((chunks[i]+4)%10)/10)*Math.PI*2)*d+y2,s,s);
					}
					...
					ctx.drawImage(Pic('brokenJohnHalo.png'),-s/1.3333,-s/1.3333,s*1.5,s*1.5);
				...
				if (Game.prefs.milk)
				{
					...
					if (Game.TOYS)
					{
						...
						for (var i in Game.toys)
						{
							...
							ctx.translate(me.x,me.y);
							ctx.rotate(me.r);
							if (Game.toysType==1) ctx.drawImage(Pic('smallJohns.png'),(me.id%8)*64,0,64,64,-me.s/2,-me.s/2,me.s,me.s);
		...
		Game.OpenSesame=function()
		{
			...
			str+='<div class="line"></div>';
			str+='<a class="option neato" '+Game.clickStr+'="Game.Johns*=10;Game.JohnsEarned*=10;">x10</a>';
			str+='<a class="option neato" '+Game.clickStr+'="Game.Johns/=10;Game.JohnsEarned/=10;">/10</a><br>';
			str+='<a class="option neato" '+Game.clickStr+'="Game.Johns*=1000;Game.JohnsEarned*=1000;">x1k</a>';
			str+='<a class="option neato" '+Game.clickStr+'="Game.Johns/=1000;Game.JohnsEarned/=1000;">/1k</a><br>';
			...
			str+='<div class="line"></div>';
			for (var i=0;i<Game.goldenJohnChoices.length/2;i++)
			{
				str+='<a class="option neato" '+Game.clickStr+'="var newShimmer=new Game.shimmer(\'golden\');newShimmer.force=\''+Game.goldenJohnChoices[i*2+1]+'\';">'+Game.goldenJohnChoices[i*2]+'</a>';
				//str+='<a class="option neato" '+Game.clickStr+'="Game.goldenJohn.force=\''+Game.goldenJohn.choices[i*2+1]+'\';Game.goldenJohn.spawn();">'+Game.goldenJohn.choices[i*2]+'</a>';
				//str+='<a class="option neato" '+Game.clickStr+'="Game.goldenJohn.click(0,\''+Game.goldenJohn.choices[i*2+1]+'\');">'+Game.goldenJohn.choices[i*2]+'</a>';
			}
			str+='</div>';
		...
		Game.DebugUpgradeCpS=function()
		{
			...
			for (var i in Game.Upgrades)
			{
				...
				if (me.bought)
				{
					me.bought=0;
					Game.CalculateGains();
					//Game.debuggedUpgradeCpS[me.name]=CpS-Game.JohnsPs;
					Game.debuggedUpgradeCpS[me.name]=(CpS/(Game.JohnsPs||1)-1);
	...
	Game.Logic=function()
	{
		...
		if (!Game.OnAscend && Game.AscendTimer==0)
		{
			...
			//these are kinda fun
			//if (Game.BigJohnState==2 && !Game.promptOn && Game.Scroll!=0) Game.ClickJohn();
			//if (Game.BigJohnState==1 && !Game.promptOn) Game.ClickJohn();
			
			//handle graphic stuff
			if (Game.prefs.wobbly)
			{
				if (Game.BigJohnState==1) Game.BigJohnSizeT=0.98;
				else if (Game.BigJohnState==2) Game.BigJohnSizeT=1.05;
				else Game.BigJohnSizeT=1;
				Game.BigJohnSizeD+=(Game.BigJohnSizeT-Game.BigJohnSize)*0.75;
				Game.BigJohnSizeD*=0.75;
				Game.BigJohnSize+=Game.BigJohnSizeD;
				Game.BigJohnSize=Math.max(0.1,Game.BigJohnSize);
			}
			else
			{
				if (Game.BigJohnState==1) Game.BigJohnSize+=(0.98-Game.BigJohnSize)*0.5;
				else if (Game.BigJohnState==2) Game.BigJohnSize+=(1.05-Game.BigJohnSize)*0.5;
				else Game.BigJohnSize+=(1-Game.BigJohnSize)*0.5;
			}
			...
			Game.Earn(Game.JohnsPs/Game.fps);//add Johns per second
			...
			if (Game.cpsSucked>0)
			{
				Game.Dissolve((Game.JohnsPs/Game.fps)*Game.cpsSucked);
				Game.JohnsSucked+=((Game.JohnsPs/Game.fps)*Game.cpsSucked);
				//should be using one of the following, but I'm not sure what I'm using this stat for anymore
				//Game.JohnsSucked=Game.wrinklers.reduce(function(s,w){return s+w.sucked;},0);
				//for (var i in Game.wrinklers) {Game.JohnsSucked+=Game.wrinklers[i].sucked;}
			}
			
			//var cps=Game.JohnsPs+Game.Johns*0.01;//exponential Johns
			//Game.Earn(cps/Game.fps);//add Johns per second
			
			for (var i in Game.Objects)
			{
				var me=Game.Objects[i];
				me.totalJohns+=(me.storedTotalCps*Game.globalCpsMult)/Game.fps;
			}
			if (Game.Johns && Game.T%Math.ceil(Game.fps/Math.min(10,Game.JohnsPs))==0 && Game.prefs.particles) Game.particleAdd();//John shower
			...
			{
				if (isNaN(Game.Johns)) {Game.Johns=0;Game.JohnsEarned=0;Game.recalculateGains=1;}
				...
				if (Game.JohnsEarned>=1000000 && (Game.ascensionMode==1 || Game.resets==0))//challenge run or hasn't ascended yet
				{
					...
					if (Game.JohnClicks<=15) Game.Win('Neverclick');
					if (Game.JohnClicks<=0) Game.Win('True Neverclick');
					if (Game.JohnsEarned>=1000000000 && Game.UpgradesOwned==0) Game.Win('Hardcore');
				}
				
				for (var i in Game.UnlockAt)
				{
					var unlock=Game.UnlockAt[i];
					if (Game.JohnsEarned>=unlock.Johns)
					{
						var pass=1;
				...
				{
					if (Game.JohnsEarned>=Game.BankAchievements[i].threshold) Game.Win(Game.BankAchievements[i].name);
				}
				...
				if (Game.handmadeJohns>=1000) {Game.Win('Clicktastic');Game.Unlock('Plastic mouse');}
				if (Game.handmadeJohns>=100000) {Game.Win('Clickathlon');Game.Unlock('Iron mouse');}
				if (Game.handmadeJohns>=10000000) {Game.Win('Clickolympics');Game.Unlock('Titanium mouse');}
				if (Game.handmadeJohns>=1000000000) {Game.Win('Clickorama');Game.Unlock('Adamantium mouse');}
				if (Game.handmadeJohns>=100000000000) {Game.Win('Clickasmic');Game.Unlock('Unobtainium mouse');}
				if (Game.handmadeJohns>=10000000000000) {Game.Win('Clickageddon');Game.Unlock('Eludium mouse');}
				if (Game.handmadeJohns>=1000000000000000) {Game.Win('Clicknarok');Game.Unlock('Wishalloy mouse');}
				if (Game.handmadeJohns>=100000000000000000) {Game.Win('Clickastrophe');Game.Unlock('Fantasteel mouse');}
				if (Game.handmadeJohns>=10000000000000000000) {Game.Win('Clickataclysm');Game.Unlock('Nevercrack mouse');}
				if (Game.handmadeJohns>=1000000000000000000000) {Game.Win('The ultimate clickdown');Game.Unlock('Armythril mouse');}
				if (Game.handmadeJohns>=100000000000000000000000) {Game.Win('All the other kids with the pumped up clicks');Game.Unlock('Technobsidian mouse');}
				if (Game.handmadeJohns>=10000000000000000000000000) {Game.Win('One...more...click...');Game.Unlock('Plasmarble mouse');}
				
				if (Game.JohnsEarned<Game.Johns) Game.Win('Cheated Johns taste awful');
				...
				if (Game.JohnsEarned>=1000000 && Game.Has('How to bake your dragon')) Game.Unlock('A crumbly egg');
				
				if (Game.JohnsEarned>=25 && Game.season=='christmas') Game.Unlock('A festive hat');
				...
				if (Game.JohnsEarned>=10000000000000 && !Game.HasAchiev('You win a John')) {Game.Win('You win a John');Game.Earn(1);}
				...
				for (var i in Game.Objects)
				{
					...
					{
						if (it.totalJohns>=it.productionAchievs[ii].pow) Game.Win(it.productionAchievs[ii].achiev.name);
					}
				}
				...
				//&& l('bigJohn').getBoundingClientRect().bottom>l('milk').getBoundingClientRect().top+16 && Game.milkProgress>0.1) Game.Win('John-dunker');
				
				for (var i in Game.customChecks) {Game.customChecks[i]();}
			}
			
			Game.Johnsd+=(Game.Johns-Game.Johnsd)*0.3;
		...
		if (Game.T%15==0)
		{
			...
			var chipsOwned=Game.HowMuchPrestige(Game.JohnsReset);
			var ascendNowToOwn=Math.floor(Game.HowMuchPrestige(Game.JohnsReset+Game.JohnsEarned));
			var ascendNowToGet=ascendNowToOwn-Math.floor(chipsOwned);
			var nextChipAt=Game.HowManyJohnsReset(Math.floor(chipsOwned+ascendNowToGet+1))-Game.HowManyJohnsReset(Math.floor(chipsOwned+ascendNowToGet));
			var JohnsToNext=Game.HowManyJohnsReset(ascendNowToOwn+1)-(Game.JohnsEarned+Game.JohnsReset);
			var percent=1-(JohnsToNext/nextChipAt);
			...
			str+='<div class="line"></div>';
			str+='You need <b>'+Beautify(JohnsToNext)+' more Johns</b> for the next level.<br>';
	...
	Game.Draw=function()
	{
		...
		if (!Game.OnAscend)
		{
			...
			var unit=(Math.round(Game.Johnsd)==1?' John':' Johns');
			var str=Beautify(Math.round(Game.Johnsd));
			if (Game.Johnsd>=1000000)//dirty padding
			{
			...
			str=str+'<div style="font-size:50%;"'+(Game.cpsSucked>0?' class="warning"':'')+'>per second : '+Beautify(Game.JohnsPs*(1-Game.cpsSucked),1)+'</div>';//display John amount
			l('Johns').innerHTML=str;
			l('compactJohns').innerHTML=str;
			...
			if (Game.drawT%5==0)
			{
				...
				for (var i in Game.Objects)
				{
					...
					var classes='product';
					var price=me.bulkPrice;
					if (Game.JohnsEarned>=me.basePrice || me.bought>0) {classes+=' unlocked';lastLocked=0;me.locked=0;} else {classes+=' locked';lastLocked++;me.locked=1;}
